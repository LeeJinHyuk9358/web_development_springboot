...
import me.leejinhyuk.springbootdeveloper.repository.BlogRepository;
    ...
    public void mockMvcSetUp() {
      ...
      /*
        ...
        : 이 클래스로 만든 자바 객체를 JSON 데이터로 변환하는 것 -> 직렬화
            ...
            HTTP 에서는 JSON을,자바에서는 객체를 사용하는데, 서로 방식이 다르기 때문에
            형식에 맞게 변환하는 과정이 필요합니다. 이 작렬화/역직렬화

                1) 직렬화 Java 시스템 내부에서 사용되는 객체를 외부에서 사용하도록
                    변환하는 과정ㅇ르 뜻하는데,
                    예를 들어 title은 "제목", "content"는 "내용"이라는 값이 들어가있는
                    개체가 있다고 가정했을 때,
                    자바 상에서는
