...
/*
    New project 생성시 주의점 :
        ...
        4. build.gradle 설정을 복사하는데, 복사 후에 -> sync를 꼭꼭 눌러주셔야 합니다.
            -> SpringBootDeveloperApplication에서 @SpringBootApplication 애너테이션에 빨간줄 가 있다면
                새로고침 안했을 확률이 거의 90%
        5. resources 내에 static 내에 index.html이라고 하는데, 해당 폴더명의 경우 대부분의 개발자들이 합의한 형태
        6. 앞으로의 수업 시간에는 new project 생서시에 이렇게 자세히 풀이할 일은 자주 있지 않을 예정이니 꼭
...
/*
    1. Spring vs. SpringBoot
        ...
        Enterprise Application : 대규ㅜ모의 복잡한 데이터를 관리하는 애플리케이션
        ...
        SpringBoot의 등장
            ...
            차이점 :
                1) 구성의 차이 -> 스플잉느 애플리케이션 개발에 필요한 환경을 수동으로 구성하고 정의
        ...
        3) 빈과 스프링 컨테이너
            ...
            3)-2. 빈 : 스프링 컨테이너가 생성하고 관리하는 객체 -> 이상의 코드들에서 B가 빈에 해당
                ...
                public class MyBean{}
                    ...
                    이후 스프링 컨테이너에서 이 클래스를 관리하게 되고, 빈으 ㅣ이름은 첫 문자를
                    소문자로 바꾸어서 관리함. 즉 클래스 MyBean의 빈 이름은 myBean이 됨.

                    일반적으로는 스프링이 제공해주는 객객체로 받아들이시면 됩니다 !!!
                    
            4) 관점 지향 프로그래밍(AOP)
                Aspect-oriented Programming) : 프로그램에 대한 관심을 핵심 관점 / 부가 관점
                    으로 나누어 모듈화함을 의미.
...
3. 스프링 부트 3 둘러보기
    첫 번째 스프링 부트 3 예제 만들기
     ...
     스프링 부트 스타터 살펴보기
        ...
        스프링 부트 스타터는 의존성이 모여 있는 그룹에 해당함.
