...
/*
    요청 헤더에서 키가 'Authorization'인 필드의 값을 가져온 다음 토큰의 접두사 Bearer를 제거한 값을 얻습니다.
    만약 값이 null이거나 Bearer로 시작되지 않으면 null을 반환(getAccessToken()를 토오가했냐 아니냐의 의미죠).
    ...
    유저 객체에는 유저이름(username)과 권한 목록(Authoriities)과 같은 인증 정보가 포함.
