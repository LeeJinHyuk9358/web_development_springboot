...
-- user에서 country가 Korea인 회원 중에서 마케팅 수신에 동의한 회원 수를 구해 출력할 것
...
-- user에서 country별로 마케팅 수신 동의한 회원 수와 동의하지 않은 회원 수를 구해 출력
-- 표시 컬럼 country, uniqueUserCnt (Group by - 00)
...
-- 2. orders에서 staff_id 별, user_id 별로 주문 건수(COUNT(*))를 출력할 것.
	-- 단, staff_id 기준 오름 차순 주문 건수 기준 내림차순
...
-- SELECT country, count(DISTINCT id)
-- 	FROM users
-- 	WHERE count(DISTINCT id) >= 8;		 오류 나는 사례
...
-- WHERE과 동일하게 필터링 기능을 수행하지만, 적용 영역의 차이 떄문에 주의할 필요 있음.
...
3. users에서 USA, Brazil, Korea, Argentina, Mexico에 거주 중인 회원 수를
	국가별로 추출(단, 회원 수가 5명 이상인 국가만 추출하고, 회원 수 기준으로 내림 차순)
