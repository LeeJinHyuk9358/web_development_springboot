...
-- 이를 JOIN 시켰을 때 u.id로 ORDER BY 하더라도 합친 테이블 상에서 제 1 컬럼(즉 PK 컬럼)에
...
-- 이상의 예시에서는 user 테이블이 왼쪽에 있으므로, users 테이블의 모든 결과값을 가져오는데,
-- orders 테이블에 대응하는 값이 없는 경우 null 값으로 출력됩니다.

-- 즉, 두 테이블의 교집합과 교집합에 속하지 않는 왼쪽 차집합을 불러옵니다.
-- 왼쪽 테이블의 값 전부 불러오기 때문에 LEFT JOIN / LEFT OUTER JOIN이라고 합니다.
...
-- u.phone, o.user_id, o.id, od.order_id, od.product_id).
...
-- RIGHT JONI은 기본적으로 LEFT JOIN과 기능은 동일합니다. LEFT JOIN에서는 왼쪽에 위치한
...
-- 이상을 이유로,
-- 많은 기업에서는 RIGHT JOIN 대신에 LEFTJOIN을 사용하도록 권장합니다.
...
-- 직원 아이디, 직원 성, 주문 아이디, 주문 일자만 출력하세요)
