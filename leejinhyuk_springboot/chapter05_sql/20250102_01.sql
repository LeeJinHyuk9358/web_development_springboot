...
-- 제한은 없음. 논리연선 AND / OR / BETWEEN
	
-- 예제 : 회원 정보 테이블 users에서 가입 일시(created_at)가
-- 2010-12-01부터 2011-01-01까지인 회원 정보를 출력해보세요
	
SELECT * FROM users WHERE created_at BETWEEN "2010-12-10" AND "2011-01-01";
...
SELECT * FROM users WHERE country = "Korea" OR "USA" OR "UK";
...
-- LIKE : 해당 전치사 뒤의 작은따옴표/큰따옴표 내에는 와일드카드를 사용 가능
...
SELECT * FROM users WHERE country != "Korea" AND country != "Canada" AND country != "Belgium";
...
-- 4. 2 개 이상의 정렬 기준을 지정할 때 각각 지정 가능
	-- 이상의 경우 각 컬럼 당 명시적으로 ASC 혹은 DSEC를 지정 해줘야 함.
...
SELECT *
	FROM orderdetails 
	ORDER BY product_id DESC, quantity ASC;		-- ASC 적어도 되고 안적어도 되고(default는 오름차순이기 때문)
